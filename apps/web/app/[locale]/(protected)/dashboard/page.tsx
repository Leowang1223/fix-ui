...
export default function DashboardPage() {
  ...
  const todayGoal = stats.streak > 0 ? t('keepStreak') : t('completeLesson')
  ...
  return (
    <div className="w-full h-full flex flex-col text-slate-900 overflow-x-hidden">
      <div className="flex-1 space-y-6 sm:space-y-8 overflow-x-hidden">
        ...
        <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-600 p-6 sm:p-8 text-white shadow-xl">
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-4 py-1.5 mb-4">
              <Target size={16} />
              <span className="text-sm font-medium">{t('todaysGoal')}</span>
            </div>
            ...
            {nextLesson && (
              <p className="text-blue-100 text-sm sm:text-base mb-6">
                {nextLessonProgress > 0
                  ? t('continueLesson', { title: nextLesson.title, progress: nextLessonProgress })
                  : t('nextUp', { title: nextLesson.title })
                }
              </p>
            )}
        ...
        >
          ...
          <div className="overflow-x-auto -mx-3 px-3 sm:mx-0 sm:px-0 scrollbar-thin">
            ...
            <StatCard
              ...
              color="purple"
            />
            <Tooltip content={t('streakTooltip')} position="bottom">
              <StatCard
                icon={Flame}
                label={t('streak')}
        ...
        <section className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-slate-800">{t('learningTrend')}</h3>
            <span className="text-xs text-slate-400">{t('last7Days')}</span>
          </div>
        ...
        <section className="space-y-3">
          <h3 className="font-semibold text-slate-800">{t('quickActions')}</h3>
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div className="text-left">
                <p className="text-sm font-bold text-indigo-800">{t('learningPathAction')}</p>
                <p className="text-xs text-indigo-600">{t('browseChapters')}</p>
              </div>
            </motion.button>
            ...
            >
              ...
              <div className="text-left">
                <p className="text-sm font-bold text-emerald-800">{t('aiConversationAction')}</p>
                <p className="text-xs text-emerald-600">{t('practiceSpeaking')}</p>
              </div>
            </motion.button>
            ...
            >
              ...
              <div className="text-left">
                <p className="text-sm font-bold text-sky-800">{t('flashcardsAction')}</p>
                <p className="text-xs text-sky-600">{t('reviewVocabulary')}</p>
              </div>
            </motion.button>
            ...
            >
              ...
              <div className="text-left">
                <p className="text-sm font-bold text-amber-800">{t('historyAction')}</p>
                <p className="text-xs text-amber-600">{t('reviewProgress')}</p>
              </div>
            </motion.button>
          </div>
        </section>
      ...
      <PageGuide
        ...
        steps={[
          {
            title: tGuide('learningOverview'),
            description: tGuide('learningOverviewDesc'),
            icon: TrendingUp,
          },
          {
            title: tGuide('dailyGoals'),
            description: tGuide('dailyGoalsDesc'),
            icon: Target,
          },
          {
            title: tGuide('quickActionsGuide'),
            description: tGuide('quickActionsGuideDesc'),
            icon: Sparkles,
          },
          {
            title: tGuide('smartRec'),
            description: tGuide('smartRecDesc'),
            icon: BookOpen,
          },
        ]}
      />
