      ...
      <div className="space-y-4">
        ...
        <div className="bg-gray-100 rounded-lg p-4">
          <div className="flex justify-between items-center">
            <span className="text-gray-600">{t('totalPractices')}</span>
        ...
        {/* 最後練習時間 */}
        <div className="bg-gray-100 rounded-lg p-4">
          <div className="text-sm text-gray-600 mb-1">{t('lastPractice')}</div>
      ...
      {latestScore && (
        <div className="mt-6 pt-6 border-t border-gray-200">
          ...
          <div className={`rounded-lg p-4 text-center mb-4 ${
            ...
            <div className="text-sm text-gray-600 mb-1">{t('thisAttempt')}</div>
          ...
          <div className="space-y-2 mb-4">
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">{t('pronunciation')}</span>
              <span className="font-bold text-blue-600">{latestScore.detailedScores.pronunciation}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">{t('fluency')}</span>
            ...
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">{t('comprehension')}</span>
            ...
            <div className="bg-blue-50 rounded-lg p-4 mb-4">
              ...
              <div className="space-y-2 text-xs text-gray-700">
                {latestScore.suggestions.pronunciation && (
                  <div><strong>{t('pronunciation')}</strong> {latestScore.suggestions.pronunciation}</div>
                )}
                {latestScore.suggestions.fluency && (
                  <div><strong>{t('fluency')}</strong> {latestScore.suggestions.fluency}</div>
                )}
                {latestScore.suggestions.accuracy && (
                  <div><strong>{t('accuracy')}</strong> {latestScore.suggestions.accuracy}</div>
                )}
              </div>
            </div>
          )}
